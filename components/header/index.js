...
import Wish from "../Wish"
        ...
        <Accordion.Panel>
         {
          item.description.map( desc => (
           <Link key={desc} href={`/category/${item.title}`}>
            <UnstyledButton className={classes.subLink} m={10} key={desc} onClick={click}>
  ...
  console.log(router.query.country)
    return (
      <>
        <Header height={80}  px="md" style={{position:'fixed',background:state,transition:0.1,zIndex:1,margin:5,borderRadius:15,borderBottom:'none'}}>
          <Group position="apart" sx={{ height: "100%" }}>
           ...
           {
            !user &&  
            <Link className="next" href="/login"> 
