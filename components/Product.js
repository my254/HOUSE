...
import {
    ...
    ScrollArea,
    Modal,
    NumberInput,TextInput,Select, Divider, Menu
  } from "@mantine/core"
...
import { useForm } from "@mantine/form"
  ...
  export default function FeaturesCard({data,admin,search}) {
    const regex = '/[\[\]"]+/g'
    ...
    const addWish = () => {
      ...
      showNotification({
        ...
        message:'A product has been added to your wish list'
      })
    }
    const genNewId = () => {
  ...
  const SelectItem = forwardRef(
   ...
   useEffect(() => {
    ...
    <div>
      {
        search ? <Menu.Item my={10} key={data.name} onClick={() => SetOpen(true)} rightSection={<IconChevronRight size={14} stroke={1.5} />}>
        <Group>
          <Avatar
            radius="xl"
            src={data.images[0].replace(/[\[\]"]+/g,'')}
          />

          <div>
            <Text weight={500}>{data.name.slice(0,30)}</Text>
            <Text size="xs" color="dimmed">
