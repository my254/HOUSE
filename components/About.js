...
export default function UserCardImage({name,email,hero,dp,reset,data:user_state}) {
    ...
    const form = useForm({
      ...
      const [heroo, setHero] = useState(null)
      const [show,setShow] = useState(false)
      const [showH,setShowH] = useState(false)
      const cropperRef = useRef()
      const cropperRefH = useRef()
      const crop = () => {
      ...
      const cropH = () => {
        //console.log(preview,cropperRef.current?.getCanvas().toDataURL())
        if(cropperRefH.current?.getCanvas()){
          let data = {
            imageBase64:cropperRefH.current?.getCanvas().toDataURL(),
            fullName:uuidv4(),
            id:uuidv4()
          }
          //function to convert base64 string to blob file
 ...
 const data = 
 {
  ...
  return (
      ...
      </Group>
    {
      ...
      <FileButton onChange={setHero} accept="image/png,image/jpeg">
          {(props) => (
              <Button
              {...props}
              radius="md"
